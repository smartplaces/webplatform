...
},{except:['entrySignIn','entrySignUp','entrySignOut','entryForgotPassword','entryResetPassword']});
...
Router.map(function (){
  ...
  this.route('scenarios',{
    ...
    waitOn: function () {return [Meteor.subscribe('scenarios'),Meteor.subscribe('messages'),Meteor.subscribe('coupons')];}
  });

  this.route('newscenario',{
    path:'/scenarios/new',
    template:'scenarioPage',
    waitOn: function () {return [Meteor.subscribe('scenarios'),Meteor.subscribe('tags'),Meteor.subscribe('messages'),Meteor.subscribe('coupons')];}
  });
